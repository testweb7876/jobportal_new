...
export default function AdminLayout() {
  ...
  const superAdminNav = [
    { icon: BarChart3,    label: 'Revenue',         href: '/admin/revenue',     divider: true },
    { icon: TrendingUp,   label: 'Analytics',       href: '/admin/analytics' },
    { icon: Shield,       label: 'Admin Accounts',  href: '/admin/admins' },
    { icon: Package,      label: 'Packages',        href: '/admin/packages' },
    { icon: RefreshCw,    label: 'Refunds',         href: '/admin/refunds' },
    { icon: Send,         label: 'Broadcast',       href: '/admin/broadcast' },
    { icon: Tag,          label: 'Categories',      href: '/admin/categories' },
  ]

  const navItems = isSuperAdmin ? [...sharedNav, ...superAdminNav] : sharedNav
    ...
    <div className="flex flex-col h-full bg-dark-900">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className={clsx(
              ...
              isSuperAdmin ? 'text-amber-400' : 'text-primary-400'
            )}>
      ...
      <nav className="flex-1 p-3 space-y-0.5 mt-2 overflow-y-auto">
        {navItems.map((item, index) => {
          ...
          return (
            <div key={item.href}>
              {item.divider && isSuperAdmin && (
                <div className="flex items-center gap-2 px-3 py-2 mt-2">
                  <div className="h-px flex-1 bg-dark-700" />
                  <span className="text-xs text-amber-500 font-medium">Superadmin</span>
                  <div className="h-px flex-1 bg-dark-700" />
                </div>
              )}
