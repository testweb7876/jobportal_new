...
export default function EditResume() {
    ...
    if (data) {
      reset({
        ...
        institutes: data.institutes?.length
          ? data.institutes
          : [{ institute: '', instituteCertificateName: '', instituteStudyArea: '', fromDate: '', toDate: '' }],
        employers: data.employers?.length
          ? data.employers
          : [{ employer: '', employerPosition: '', employerCity: '', employerFromDate: '', employerToDate: '', employerCurrentStatus: 0 }],
  ...
  const updateMutation = useMutation({
    mutationFn: (payload) => api.patch(`/resumes/${id}`, payload),
    onSuccess: () => {
      toast.success('Resume updated!')
      qc.invalidateQueries(['resume', id])
      qc.invalidateQueries(['my-resumes'])
  ...
  return (
    <div className="max-w-3xl space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <Link to={`/jobseeker/resumes/${id}`} className="text-gray-500 hover:text-gray-900 dark:hover:text-white">
          <ArrowLeft size={18} />
        </Link>
        <div>
          <h1 className="page-title mb-0">Edit Resume</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">Update your resume details below</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="card p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button"
              onClick={() => addInstitute({ institute: '', instituteCertificateName: '', instituteStudyArea: '', fromDate: '', toDate: '' })}
        ...
        <div className="card p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button"
              onClick={() => addEmployer({ employer: '', employerPosition: '', employerCity: '', employerFromDate: '', employerToDate: '', employerCurrentStatus: 0 })}
              className="btn-outline btn-sm"><Plus size={13} /> Add</button>
          </div>
        ...
        <div className="flex justify-end gap-3">
          <Link to={`/jobseeker/resumes/${id}`} className="btn-secondary">Cancel</Link>
