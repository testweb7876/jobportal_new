...
export default function EmpMessages() {
  ...
  const { data: conversations } = useQuery({
    queryKey: ['emp-conversations'],
    queryFn: () => messageAPI.getConversations().then(r => r.data?.data || []),
  })

  const { data: messagesData } = useQuery({
    queryKey: ['emp-messages', selected?._id],
  ...
  useEffect(() => {
    ...
    socket.on('new_message', ({ conversationId }) => {
      qc.invalidateQueries({
        queryKey: ['emp-messages', conversationId]
      })
      qc.invalidateQueries({
        queryKey: ['emp-conversations']
      })
  ...
  // Auto-select conversation from navigate state
  useEffect(() => {
    if (location.state?.conversationId && convos.length > 0) {
      const target = convos.find(c => c._id === location.state.conversationId)
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex items-center gap-3">
                        <Avatar name={`${other?.firstName} ${other?.lastName}`} src={other?.avatar?.secureUrl} size="sm" />
                        <p className="font-semibold text-sm text-gray-900 dark:text-white">{other?.firstName} {other?.lastName}</p>
                      </div>
                    )
                  })()}
                </div>
...
function MessageInput({ conversationId }) {
  ...
  return (
    <div className="p-4 border-t border-gray-100 dark:border-dark-700 flex gap-3">
      ...
      />
      <button onClick={send} className="btn-primary px-4 h-12">
        <Send size={15} />
      </button>
    </div>
  )
