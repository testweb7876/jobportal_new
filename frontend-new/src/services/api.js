...
// resumeAPI defined below in full version
...
// ─── MISSING APIs - Added for full backend alignment ─────────────────────────
...
export const resumeAPI = {
  ...
  getByShareToken: (token) => api.get(`/resumes/share/${token}`),
  ...
  deleteFile: (id, publicId) => api.delete(`/resumes/${id}/files/${encodeURIComponent(publicId)}`),
  updateVisibility: (id, data) => api.patch(`/resumes/${id}/visibility`, data),
  generateShareLink: (id) => api.post(`/resumes/${id}/share`),
}

// Company gallery (missing)
export const companyGalleryAPI = {
  uploadImage: (formData) => api.post('/companies/gallery', formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
  deleteImage: (publicId) => api.delete('/companies/gallery', { data: { publicId } }),
  adminVerify: (id, data) => api.patch(`/companies/${id}/verify`, data),
}
...
export const messageDeleteAPI = {
  deleteMessage: (messageId, data) => api.delete(`/messages/${messageId}`, { data }),
}

// Analytics (missing in frontend)
export const analyticsAPI = {
  employer: () => api.get('/analytics/employer'),
  jobseeker: () => api.get('/analytics/jobseeker'),
}
...
export const adminAPIExtra = {
  ...
  verifyCompany: (id, data) => api.patch(`/companies/${id}/verify`, data),
}
